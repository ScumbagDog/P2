...
public class MainGUI extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        accessBar.setFileMenuItemFunctionality(graphicsManager.getSelectedFiles(), graphicsManager);
        setCompareButtonFunctionality();
        initiateResultList();

        graphicsManager.updateDisplay();
        Scene scene = new Scene(graphicsManager.getElementHolder(), settings.getWindowWidth(), settings.getWindowHeight());
    ...
    private void setCompareButtonFunctionality(){
        ...
        compareMelodies.setOnAction(event -> {
            ...
            Text confirmationText = new Text("A total of " + graphicsManager.getSelectedFiles().getSrcMidiFiles().size() + " source files and "
                    + graphicsManager.getSelectedFiles().getCompMidiFiles().size() + " comparison files have been selected.\n" +
                    "Do you want to begin the comparison sequence?");
            Button startComparison = new Button("Start");
            startComparison.setOnAction(event2 -> {comparison.useUkonnen(resultlist,
                    graphicsManager.getSelectedFiles().getSrcMidiFiles(),
                    graphicsManager.getSelectedFiles().getCompMidiFiles());});
    ...
    private void initiateResultList(){
        graphicsManager.getResultSplit().setOrientation(Orientation.VERTICAL);
        graphicsManager.getResultSplit().setDividerPositions(0.9);
        resultStack2.setMaxSize(100, 100);
        graphicsManager.getResultSplit().getItems().addAll(resultStack1, resultStack2);
    }
}